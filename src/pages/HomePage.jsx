...
const HomePage = () => {
  ...
  const loadRooms = async () => {
    ...
    try {
      const expiredBookings = await listRoomsTwoHoursAgo();
      if (expiredBookings.rows.length) {
        const bookingCleanupPromises = expiredBookings.rows.map((booking) =>
          removeGuest(booking.$id)
        );

        await Promise.all(bookingCleanupPromises);
      }

      const roomsAfterToday = await listRoomsAfterToday();
      if (roomsAfterToday.rows.length) {
        const roomsCleanupPromises = roomsAfterToday.rows.map((room) =>
          removeGuest(room.$id)
        );

        await Promise.all(roomsCleanupPromises);
      }
  ...
  return (
    <div className="mx-auto min-h-screen flex flex-col md:mx-5 mt-4">
      ...
      <main className="grow px-6 py-4 overflow-y-auto">
